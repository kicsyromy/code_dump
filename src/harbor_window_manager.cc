...
namespace
{
    unsigned int const shift_states = mir_input_event_modifier_alt |
                                      mir_input_event_modifier_shift |
                                      mir_input_event_modifier_sym | mir_input_event_modifier_ctrl |
                                      mir_input_event_modifier_meta;
}

bool WindowManagerPolicy::handle_pointer_event(MirPointerEvent const *event)
{
    auto const action = mir_pointer_event_action(event);
    auto const shift_state = mir_pointer_event_modifiers(event) & shift_states;
    Point const cursor{ mir_pointer_event_axis_value(event, mir_pointer_axis_x),
                        mir_pointer_event_axis_value(event, mir_pointer_axis_y) };

    Displacement movement{ mir_pointer_event_axis_value(event, mir_pointer_axis_relative_x),
    ...
    switch (pointer_gesture)
    {
        case pointer_gesture_moving:
            if (action == mir_pointer_action_motion && shift_state == pointer_gesture_shift_keys &&
        ...
        case pointer_gesture_resizing:
                ...
                mir_pointer_event_button_state(event, pointer_gesture_button))
            {
                    ...
                    tools.select_active_window(pointer_gesture_window) == pointer_gesture_window)
                {
                    ...
                    keep_size_within_limits(tools.info_for(pointer_gesture_window), movement,
                    ...
                    if (resize_edge & mir_resize_edge_west) new_pos.x = top_left.x + movement.dx;

                    if (resize_edge & mir_resize_edge_north) new_pos.y = top_left.y + movement.dy;
    ...
    if (!consumes_event)
    {
        switch (action)
        {
            case mir_pointer_action_button_down:
                if (auto const window = tools.window_at(cursor)) tools.select_active_window(window);
                break;
...
bool WindowManagerPolicy::handle_touch_event(MirTouchEvent const *event)
{
    ...
    for (auto i = 0U; i != count; ++i)
    {
        for (auto j = 0U; j != i; ++j)
        {
            ...
            if (touch_pinch_width < dx) touch_pinch_width = dx;

            if (touch_pinch_height < dy) touch_pinch_height = dy;
        }
        ...
        if (touch_pinch_top > y) touch_pinch_top = y;

        if (touch_pinch_left > x) touch_pinch_left = x;
    }
    ...
    if (is_drag)
    {
        if (count == 3)
        {
            if (auto window = tools.active_window())
            {
                ...
                Displacement delta{ DeltaX{ touch_pinch_left - old_touch_pinch_left },
    ...
    if (!consumes_event && pinching) end_touch_gesture();
...
bool WindowManagerPolicy::handle_keyboard_event(MirKeyboardEvent const *event)
{
...
void WindowManagerPolicy::end_touch_gesture()
{
    if (!pinching) return;
...
void WindowManagerPolicy::keep_size_within_limits(WindowInfo const &window_info,
                                                  Displacement &delta,
                                                  Width &new_width,
                                                  Height &new_height) const
{
    ...
    {
        new_width = min_width;
        if (delta.dx > DeltaX{ 0 }) delta.dx = DeltaX{ 0 };
    }

    if (new_height < min_height)
    {
        new_height = min_height;
        if (delta.dy > DeltaY{ 0 }) delta.dy = DeltaY{ 0 };
    }
...
Rectangle WindowManagerPolicy::confirm_placement_on_display(WindowInfo const & /*window_info*/,
...
void WindowManagerPolicy::handle_request_drag_and_drop(WindowInfo & /*window_info*/) {}

void WindowManagerPolicy::handle_request_move(WindowInfo &window_info,
                                              MirInputEvent const *input_event)
{
    begin_pointer_gesture(window_info, input_event, pointer_gesture_moving);
}

void WindowManagerPolicy::handle_request_resize(WindowInfo &window_info,
                                                MirInputEvent const *input_event,
                                                MirResizeEdge edge)
{
...
bool WindowManagerPolicy::begin_pointer_gesture(WindowInfo const &window_info,
                                                MirInputEvent const *input_event,
                                                PointerGesture gesture)
{
    if (mir_input_event_get_type(input_event) != mir_input_event_type_pointer) return false;
