...
namespace rml
{
    using event_loop_t = events::event_loop<events::event, utilities::concurrent_queue<events::event>>;
    ...
    public:
        static inline auto instance() noexcept -> application_loop &
        {
            return *instance_;
        }
    ...
    template<typename ...Args>
    class signal_t : public events::signal<application_loop, Args...>
    {
        using parent_t = events::signal<application_loop, Args...>;

    public:
        inline signal_t()
          : parent_t(application_loop::instance())
        {}
    };
