...
static void keyboard_handle_key(struct wl_listener *listener, void *data)
{
    ...
    struct wlr_event_keyboard_key *event = static_cast<wlr_event_keyboard_key *>(data);
    struct wlr_seat *seat = server->seat;
...
static void server_new_keyboard(struct tinywl_server *server, struct wlr_input_device *device)
{
    struct tinywl_keyboard *keyboard =
        static_cast<tinywl_keyboard *>(calloc(1, sizeof(struct tinywl_keyboard)));
                    ...
                    double *sy)
{
    ...
    [[maybe_unused]] struct wlr_surface_state *state = &view->xdg_surface->surface->current;
...
static void process_cursor_move(struct tinywl_server *server, [[maybe_unused]] uint32_t time)
{
    /* Move the grabbed view to the new position. */
    server->grabbed_view->x = static_cast<int>(server->cursor->x - server->grab_x);
    server->grabbed_view->y = static_cast<int>(server->cursor->y - server->grab_y);
}

static void process_cursor_resize(struct tinywl_server *server, [[maybe_unused]] uint32_t time)
{
    /*
    ...
    wlr_xdg_toplevel_set_size(view->xdg_surface, static_cast<std::uint32_t>(width),
...
static void render_surface(struct wlr_surface *surface, int sx, int sy, void *data)
{
    ...
    struct wlr_box box = {
        static_cast<int>(ox * static_cast<double>(output->scale)),
        static_cast<int>(oy * static_cast<double>(output->scale)),
...
static void server_new_output(struct wl_listener *listener, void *data)
{
    ...
    auto output = static_cast<struct tinywl_output *>(calloc(1, sizeof(struct tinywl_output)));
...
static void xdg_surface_map(struct wl_listener *listener, [[maybe_unused]] void *data)
{
...
static void xdg_surface_unmap(struct wl_listener *listener, [[maybe_unused]] void *data)
{
...
static void xdg_surface_destroy(struct wl_listener *listener, [[maybe_unused]] void *data)
{
...
static void server_new_xdg_surface(struct wl_listener *listener, void *data)
{
    ...
    auto view = static_cast<struct tinywl_view *>(calloc(1, sizeof(struct tinywl_view)));
    view->server = server;
...
int main(int argc, char *argv[])
{
    ...
    if (startup_cmd)
    {
        if (fork() == 0)
        {
            execl("/bin/sh", "/bin/sh", "-c", startup_cmd, static_cast<void *>(nullptr));
        }
    }
