...
bool Service::Process()
{
  ...
  volatile char * flags = (volatile char *)KVMFR_HEADER_FLAG_READY; //(volatile char *)&(m_shmHeader->flags);

  // check if the client has flagged a restart
  if ((int)flags & KVMFR_HEADER_FLAG_RESTART)
  {
  ...
  {
	  auto f = std::make_unique<KVMFRFrame>();
	  volatile KVMFRFrame * fi = f.get(); // &(m_shmHeader->frame);
