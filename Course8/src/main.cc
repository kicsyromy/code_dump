...
std::tuple<GLuint, GLint, GLint> compile_shaders() noexcept
{
    ...
    return { program, model_uniform, projection_uniform };
}
...
int main([[maybe_unused]] int argc, [[maybe_unused]] const char **argv)
{
    ...
    auto [program, model_uniform, projection_uniform] = compile_shaders();
    ...
    while (!quit)
    {
                ...
                case SDL_WINDOWEVENT:
                    if (event.window.event == SDL_WINDOWEVENT_RESIZED)
                    {
                        ...
                        projection = glm::perspective(
                            90.f, float(viewport_width) / float(viewport_height), .1f, 100.f);
                    }
                    break;
                }
        }
        ...
        glm::mat4 model{ 1.f };
        model = glm::translate(model, { triangle_offset, .0f, -2.5f });
